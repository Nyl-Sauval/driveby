...
class RetourController extends Controller
{
    ...
    {
        $retour = Retour::with(['location.car'])->find($id);
        if (!$retour) {
    ...
    public function update(Request $request, Retour $retour) {
        ...
        $retour->update([
            ...
            'return_fuel_level' => $validated['return_fuel_level'],
            'return_interior_status_car' => $validated['return_interior_status_car'],
            'return_exterior_status_car' => $validated['return_exterior_status_car'],
            'return_default' => $validated['return_default'] ?? null,
            'return_done' => $validated['return_done'],
        ]);

        if ($retour->location && $retour->location->car) {
            $retour->location->car->update([
